...
public interface FreeBoardMapper {
    ...
    //  페이지 + 검색 + 카테고리 + 유저타입(관리자여부) + 날짜범위 포함 목록 조회
    ...
    // 전체 데이터 개수 (유저 타입에 따라 숨김글 제외 카운트를 위해 type 추가)
    ...
    // 5. 단일 게시글 숨김 상태 변경
    // ※ 신고 관련 조회는 FreeBoardReportMapper / FreeBoardStatsMapper 에서 처리
    public void updateHiddenStatus(
            @Param("boardId") Long boardId,
            @Param("hidden")  int hidden) throws Exception;
}
