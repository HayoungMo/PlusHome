...
const UserInfo = (props) => {
	...
	const userColumns = ["id", "type", "code", "name", "email", "tel", "gender", "addr", "joined"];
	...
	const handleRowDeleteInTable = async () => {
		...
		const selectedUserList = userList.filter((row) => selectedUserKeys.includes(row.id));
	...
	const handleRowUpdateInTable = async () => {
		const changedRows = getChangedRowsDetail(userStateList, editedUserList);
		try {
		...
		return editedRows
			.map((editedRow) => {
				...
				const originRow = originRows.find(
					(originRow) => (originRow.id || originRow.c_id) === rowKey,
				);
		...
		setEditedUserList((prevList) => {
			return prevList.map((prevRow) => {
				const updatedRow = data.find(
					(row) => (row.id || row.c_id) === (prevRow.id || prevRow.c_id),
				);
	...
	const mergeEditedListWithServerList = (serverList, editedList) => {
		return serverList.map((serverRow) => {
			const editedRow = editedList.find((row) => row.id === serverRow.id);
	...
	const dialogRestoreConfirmButtonList = [
		{
			...
			color: "error",
			variant: "outlined",
			onClick: () => setRestoreConfirmOpen(!restoreConfirmOpen),
		},
		{
			title: "Restore",
			color: "primary",
			variant: "outlined",
			onClick: () => handleRowRestoreInTable(),
		},
	];
	...
	const userSearchFunction = () => {
		...
		const searchUserList = userList.filter((data) => {
			...
			//쇼핑몰 검색
			if (searchKey === "shop") {
				return data.c_kind === "shop" && data.c_name?.toLowerCase().includes(searchValue);
			}
			//인테리어 검색
			if (searchKey === "interior") {
				return (
					data.c_kind === "interior" && data.c_name?.toLowerCase().includes(searchValue)
				);
			}

			if (searchKey === "c_boss") {
				return data.c_boss?.toLowerCase().includes(searchValue);
			}
	...
	const getRelatedCompanyList = (row) => {
		return userList.filter((data) => {
			return data.code === row.code && data.c_boss === row.c_boss;
		});
	};
					...
					<>
						...
						{restoreConfirmOpen && (
							<DialogMui
								...
								onClose={() => setRestoreConfirmOpen(!restoreConfirmOpen)}
								title="Restore User"
				...
				{relatedCompanyList.length > 0 && (
					<div style={{ marginTop: "20px" }}>
						...
						<TableMui
							...
							col={userType === "user" ? userColumns : companyColumns}
							columns={
								userType === "user"
									? [
											"아이디",
