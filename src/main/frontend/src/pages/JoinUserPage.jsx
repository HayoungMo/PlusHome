...
const JoinUserPage = () => {
	...
	const onTel = (evt) => {
		...
		const newTel = { ...tel, [name]: value === "head" ? value : onlyNumber };
		setTel(newTel);
	...
	const onBirth = (evt) => {
		...
		setForm((data) => ({
			...data,
			birth: evt ? evt.format("YYYY-MM-DD") : "",
		}));
	};
	...
	const validate = (name, value) => {
		switch (name) {
			case "id":
				...
				if (!/^(?=.*[a-z])(?=.*[0-9])[a-z0-9_-]{5,20}$/.test(value)) {
					return;
				}
	...
	const validateAll = () => {
		...
		// 아이디 형식
		if (!/^(?=.*[a-z])(?=.*[0-9])[a-z0-9_-]{5,20}$/.test(form.id)) {
		...
		if (!/^(?=.*[A-Za-z])(?=.*\d)(?=.*[!@#$%^&*])[A-Za-z\d!@#$%^&*]{8,16}$/.test(form.pw)) {
			return "비밀번호 8~16자의 영문 대/소문자, 숫자, 특수문자를 사용해 주세요.";
		}
	...
	const onText = (evt) => {
		...
		if (name === "code") {
			...
			const onlyNumber = value.replace(/[^0-9]/g, "").slice(0, maxLength);
		...
		setForm((prev) => ({
			...prev,
			[name]: name === "id" ? value.slice(0, 20) : value,
		}));
	};
		...
		<div className="join-wrap">
			<div className="join-box">
				...
				<div className="join-input">
					...
					{errorMsg.id && <div className="error-msg">{errorMsg.id}</div>}

					{errorMsg.idFormat && <div className="error-msg">{errorMsg.idFormat}</div>}
				...
				<div className="join-input">
					...
					{errorMsg.pw && <div className="error-msg">{errorMsg.pw}</div>}
				</div>
				...
				{form.type === "company" && (
					<>
						...
						{errorMsg.c_name && <div className="error-msg">{errorMsg.c_name}</div>}
						...
						{errorMsg.c_addr && <div className="error-msg">{errorMsg.c_addr}</div>}
				...
				<div className="join-input">
					<TextField
						fullWidth
						label={form.type === "company" ? "[필수] 사업주명" : "[필수] 이름"}
						type="text"
						name="name"
						value={form.name}
						onChange={onText}
				...
				<div className="join-input">
					...
					{errorMsg.code && <div className="error-msg">{errorMsg.code}</div>}
				</div>
				...
				<div className="email-wrap">
					<TextField
						fullWidth
						label="[필수] 이메일"
						placeholder={email.domain === "direct" ? "example@naver.com" : ""}
						value={email.id}
						onChange={(e) => {
				...
				<div className="join-input">
					...
					{errorMsg.birth && <div className="error-msg">{errorMsg.birth}</div>}
				</div>
				...
				{errorMsg.common && <div className="error-msg">{errorMsg.common}</div>}

				{/* 버튼 */}

				<Button color="primary" variant="contained" className="join-btn" onClick={onNext}>
					가입하기
				</Button>
