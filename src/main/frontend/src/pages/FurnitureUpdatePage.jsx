...
const FurnitureUpdatePage = ({ furniture = null, onSuccess }) => {
	...
	useEffect(() => {
		const loadData = async () => {
			try {
				...
				const thumbnailImg = imageList.find((img) => img.img_tag === "THUMBNAIL");
	...
	const onDeleteOldInfoImage = (img) => {
		...
		setOldInfoImages((prev) => prev.filter((item) => item.img_name !== img.img_name));
	};

	const onDeleteOldOthersImage = (img) => {
		addDeletedImage(img.img_name);

		setOldOthersImages((prev) => prev.filter((item) => item.img_name !== img.img_name));
	};
