...
const UserQuestionPage = ({ user }) => {
	...
	const getMyQuestions = async () => {
		...
		const data = isCompanyUser
			? (
					await Promise.all(
						uniqueCompanyIds.map((c_id) => questionService.getCompanyQuestions(c_id)),
					)
				).flat()
						...
						{editIdx === item.q_idx ? (
									...
									<div key={img.img_name}>
										...
										<div>
											...
											{addImageFiles[item.q_idx]?.length > 0 && (
												<div>
													<p>
														{addImageFiles[item.q_idx].length}장의
														이미지가 추가됩니다.
													</p>
							...
							<div>
									...
									<div>
										{isCompanyUser && answerEditIdx === item.q_idx ? (
											<div>
												<TextField
													...
													onChange={(evt) =>
														onAnswerChange(item.q_idx, evt.target.value)
													}
													multiline
													rows={3}
												/>
												...
												<button
													type="button"
													onClick={() => onAnswerSubmit(item.q_idx)}>
													답변 저장
												</button>
										...
										{isCompanyUser && answerEditIdx !== item.q_idx && (
											<>
												<button
													type="button"
													onClick={() => startAnswerEdit(item)}>
													답변 수정
												</button>
												<button
													type="button"
													onClick={() => deleteAnswer(item.q_idx)}>
													답변 삭제
												</button>
											</>
										)}
								...
								{!isCompanyUser && (
									<button
										type="button"
										onClick={() => deleteQuestion(item.q_idx)}>
										삭제
									</button>
								)}
							</div>
