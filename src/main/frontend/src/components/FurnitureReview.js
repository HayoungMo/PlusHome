...
const FurnitureReview = ({ f_code, fr_idx = 0 }) => {
	...
	if (reviews.length === 0) {
		return (
			<p style={{ textAlign: "center", color: "#777", margin: 0 }}>등록된 리뷰가 없습니다.</p>
		);
	}
	...
	return (
		<div>
			{reviews?.map((item, idx) => (
				<img
					width={100}
					src={item?.logo?.result?.find((item) => item.img_tag === "THUMBNAIL")?.img_name}
					alt={`${item.c_name} 예시`}
					onClick={() => {
					...
					style={{
						...
						height: "120px",
						cursor: "pointer",
						border: selectedImage === idx ? "3px solid red" : "1px solid #ccc",
						borderRadius: "8px",
					}}
				/>
			))}
			{selectedItem && (
				<div>
					...
					<RatingMui value={selectedItem?.fr_star} readOnly={true} precision={0.5} />
				</div>
			)}
		</div>
	);
