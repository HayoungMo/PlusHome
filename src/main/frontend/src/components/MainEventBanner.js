...
const MainEventBanner = () => {
  ...
  const bannerEvents = useMemo(
    () => visibleEvents.filter((item) => getBanner(item)),
    [visibleEvents],
  );

  const currentEvent = bannerEvents[sideSlideIndex];
  const currentBanner = currentEvent ? getBanner(currentEvent) : null;

  const moveSideSlide = (direction) => {
    setSideSlideIndex((prev) => {
      if (bannerEvents.length === 0) return 0;

      if (direction === "next") {
        return prev >= bannerEvents.length - 1 ? 0 : prev + 1;
      }

      return prev <= 0 ? bannerEvents.length - 1 : prev - 1;
    });
  };
